 ...
 */
 ...
 let allMessage = '';
 let jdSignUrl = '' // 算法url
 let jdcash_Token = ''; // 算法url token 有则填
 jdSignUrl = $.isNode() ? (process.env.cash_SignUrl ? process.env.cash_SignUrl : `${jdSignUrl}`) : ($.getdata('cash_SignUrl') ? $.getdata('cash_SignUrl') : `${jdSignUrl}`);
 jdcash_Token = $.isNode() ? (process.env.cash_Token ? process.env.cash_Token : `${jdcash_Token}`) : ($.getdata('cash_Token') ? $.getdata('cash_Token') : `${jdcash_Token}`);
 if (!jdcash_Token) {
		 console.log('\n请前往 https://t.me/WALL_E_API   获取Token\n请填写获取的Token,变量是cash_Token');
     return;
 }
