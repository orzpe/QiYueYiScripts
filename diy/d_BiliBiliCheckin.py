...
def main():
    ...
    if userInfo:
        uname = userInfo["name"] # 用户名
        coin = userInfo["coins"] # 硬币数量
        current_level = userInfo["level_exp"]["current_level"] # 当前等级
        current_exp = userInfo["level_exp"]["current_exp"] # 经验值
        next_exp = userInfo["level_exp"]["next_exp"] # 下一等级所需经验值登录状态
