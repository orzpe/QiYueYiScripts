...
##小米运动 支持多账号 账号密码之间用#隔开
### 用户名（一定是要手机号登录）
#变量 xmuser="手机号#手机号"
### 登录密码
#变量 xmpasswd="密码#密码"
### 要修改的步数，直接输入想要修改的步数值，留空为随机步数
#变量 xmstep="20000-25000"

import time,re,random,os,sys,requests
...
#                 with open('sendNotify', "w+", encoding="utf-8") as f:
#                     f.write(response.text)
#             else:
#                 if a < 5:
#                     a += 1
#                     return self.getNotify(a)
#                 else:
#                     pass
#         except:
#             if a < 5:
...
if __name__ ==  "__main__":
    # 用户名（格式为 133********&187********）
    if "xmuser" in os.environ and os.environ["xmuser"]:
        user = os.environ["xmuser"]
    else:
        print("无法找到变量: xmuser, 请检查")
        exit()
    # 密码（passwd&passwd）
    if "xmpasswd" in os.environ and os.environ["xmpasswd"]:
        passwd = os.environ["xmpasswd"]
    else:
        print("无法找到变量: xmpasswd, 请检查")
        exit()
    # 步数范围（10000-12000）
    if "xmstep" in os.environ and os.environ["xmstep"]:
        step = os.environ["xmstep"]
    else:
        print("无法找到变量: xmstep, 请检查")
        exit()
    # pushplustoken（token&token）
    if "pushplus" in os.environ and os.environ["pushplus"]:
        token = os.environ["pushplus"]
    ...
    if len(user_list) == len(passwd_list):
        ...
        for line in range(0,len(user_list)):
            ...
            if len(setp_array) == 2:
                ...
                print (f"已设置随机步数为（{setp_array[0]}-{setp_array[1]}）")
            elif str(step) == '0':
                step = ''
            push = main(user_list[line], passwd_list[line], step)
            # if (token[line] != ""):
            #     pushplus(token[line],push)
            # else:
            #     print("推送关闭")
