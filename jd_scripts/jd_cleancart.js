...
默认：不执行 如需要请添加环境变量
cleancart_Run="true"
cleancart_Products="" # 账号变量
cleancart_SignUrl="" # 算法url
cleancart_Token="" # 算法url token 有则填

账号变量
——————————————
1.@&@ 前面加账号变量顺序或账号pt_pin，如果pt_pin有中文请填写中文
2.每个账号之间用“|-|”隔开
3.英文大小写请填清楚
4.优先匹配账号再匹配*
...
[Script]
cron "0 7,21 * * *" script-path=jd_cleancart.js,tag=清空购物车
*/
let jdSignUrl = '' // 算法url
...
jdSignUrl = $.isNode() ? (process.env.cleancart_SignUrl ? process.env.cleancart_SignUrl : `${jdSignUrl}`) : ($.getdata('cleancart_SignUrl') ? $.getdata('cleancart_SignUrl') : `${jdSignUrl}`);

Authorization = process.env.cleancart_Token ? process.env.cleancart_Token : `${Authorization}`
// if(Authorization && Authorization.indexOf("Bearer ") === -1) Authorization = `Bearer ${Authorization}`

cleancartRun = $.isNode() ? (process.env.cleancart_Run ? process.env.cleancart_Run : `${cleancartRun}`) : ($.getdata('cleancart_Run') ? $.getdata('cleancart_Run') : `${cleancartRun}`);

cleancartProducts = $.isNode() ? (process.env.cleancart_Products ? process.env.cleancart_Products : `${cleancartProducts}`) : ($.getdata('cleancart_Products') ? $.getdata('cleancart_Products') : `${cleancartProducts}`);
...
!(async () => {
  ...
  if(cleancartRun !== 'true'){
    console.log('脚本停止\n请添加环境变量[cleancart_Run]为"true"')
    return
  }
  if(!cleancartProducts){
    console.log('脚本停止\n请添加环境变量[cleancart_Products]\n清空商品\n内容规则看脚本文件')
    return
  }
...
async function run(){
  try{
    ...
    if(!$.out){
      ...
      waittime = parseInt(Math.random()*(40-30+1)+30)
      console.log("随机等待"+Math.round(waittime)+"秒")
...
function jdSign(fn,body) {
  ...
  return new Promise((resolve) => {
    ...
    if(Authorization) options["headers"]["token"] = Authorization
